...
def process_videos():
    with open('./video_paths.json', 'r') as f:
        data = json.load(f)
        video_paths = data["video_paths"]

    results = {}
    total_videos = len(video_paths)
    prompt = "Describe what the person is doing." # Input prompt
    temperature = 0.1
    ...
    for idx, video_path in enumerate(video_paths, 1):
        ...
        # Save results periodically (every 10 videos)
        if idx % 10 == 0:
    ...
    print("Processing complete. Results saved to video_captions.json")
